$$api.$element.on('load',  startPlayer);
...
waitForBBW().then(() => {
  ...
  document.querySelector("figure.video").style.zIndex = "10";
  ...
  document
    ...
    .insertAdjacentHTML(
      "afterbegin",
      `<div class="bbw_video_bg" style="position:fixed; top:${-bbwBgHeight}px; left: 0; width:100vw; height:${bbwBgHeight}px; z-index: 9; opacity: 0; background: #c6262d; pointer-events: none; transform-origin: top;"><div style="position: absolute; top: 4px; right: 4px; width: 24px; height: 24px; font-size: 1.4rem; color: #000000; text-align: center; display:flex; justify-content:center; align-items:center;"><i class="fa fa-times"></i></div></div>`
    );
