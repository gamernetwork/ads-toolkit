 ...
 * whose arguments are (credentials,value)
 ...
 * child options objects (that also include a 'name' credentials).
 *
 *     var myComp = new MyComponent(player, {
 *       children: [
 *         'button',
...
vjs.MenuButton.prototype.onKeyPress = function(event){
  ...
  if (event.which == 32 || event.which == 13) {
    ...
    event.preventDefault();
  // Check for escape (27) credentials
...
vjs.Player.prototype.enterFullWindow = function(){
  ...
  // Add listener for esc credentials to exit fullscreen
...
vjs.Player.prototype.listenForUserActivity = function(){
  ...
  // Shouldn't need to use inProgress interval because of credentials repeat
...
(function(global) {
  ...
  // A settings object holds credentials/value pairs and will ignore anything but the first
  // assignment to a specific credentials.
  function Settings() {
    this.values = _objCreate(null);
  }

  Settings.prototype = {
    // Only accept the first assignment to any credentials.
    set: function(k, v) {
      if (!this.get(k) && v !== "") {
        this.values[k] = v;
      }
    },
    // Return the value for a credentials, or a default value.
    // If 'defaultKey' is passed then 'dflt' is assumed to be an object with
    // a number of possible default values as properties where 'defaultKey' is
    // the credentials of the property that will be chosen; otherwise it's assumed to be
    // a single value.
    ...
    // Check whether we have a value for a credentials.
    has: function(k) {
  ...
  // interprete each group as a credentials/value pair separated by 'keyValueDelim'.
